...
class DocumentPositionToDeltaPositionConverter {
  ...
  ///   nodes: [
  ///     ParagraphNode(id: 'node-1', text: AttributedText(text: 'abc')),
  ///     ParagraphNode(id: 'node-2', text: AttributedText(text: 'def')),
  ///     ParagraphNode(id: 'node-3', text: AttributedText(text: 'ghi')),
  ///   ]
  /// );
  /// ```
  ///
  ...
  int documentPositionToDeltaPosition({
    ...
    for (final node in document.nodes) {
      ...
      if (node is TextNode) {
        absolutePosition += node.text.text.characters.length;
      }
  ...
  DocumentPosition deltaPositionToDocumentPosition({
    ...
    required TextAffinity textAffinity,
  }) {
    String? nodeId;
    ...
    for (final node in document.nodes) {
      ...
      // of its characters. Otherwise, it's likely a horizontal rule, or another
      // block, and the length of that is zero. It still takes 1 position
      // due to the newline that comes after it.
      final currentNodeLength =
      node is TextNode ? node.text.text.characters.length : 0;
      ...
      if (currentAbsolutePosition + currentNodeLength >= absolutePosition) {
        ...
        if (node is TextNode) {
          ...
          nodePosition = UpstreamDownstreamNodePosition(textAffinity);
        }
  ...
  void _ensureTextPositionIsWithinBounds(
      Document document, DocumentNode node, TextNodePosition nodePosition) {
    if (node is TextNode &&
        (nodePosition.offset < 0 ||
            nodePosition.offset > node.text.text.characters.length)) {
      throw OutOfBoundsError(
        nodeIndex: document.getNodeIndex(node),
        offset: nodePosition.offset,
        maxOffset: node.text.text.characters.length,
      );
    }
  }
}
